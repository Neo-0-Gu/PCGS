...
from .nms import nms_core


def absorb_heat(cfg, poses, heatmap_avg):
    # poses = torch.tensor(poses)[0]
    ...
    val_k, ind = kpt_heatmap.topk(cfg.DATASET.MAX_NUM_PEOPLE, dim=1)
    ...
    for i in range(num_joints):
        ...
        for p in range(keep_ind.shape[0]):
            if pose_heat_diff[p, keep_ind[p]] < cfg.TEST.MAX_ABSORB_DISTANCE:
    ...
    for batch_id in range(len(ans)):
        for joint_id in range(ans[0].shape[1]):
            ...
            for people_id in range(ans[0].shape[0]):
                if ans[batch_id][people_id, joint_id, 2] > cfg.TEST.ADJUST_THRESHOLD:
                    ...
                    try:
                        ...
                        mean_x, mean_y, value, _ = fit_gaussian_heatmap(
                            ...
                            safe_xx - safe_x_lower_bound, safe_yy - safe_y_lower_bound, cfg.DATASET.SIGMA
                        )
