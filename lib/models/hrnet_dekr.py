...
class PoseHigherResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([y_list[0], \
            F.interpolate(y_list[1], size=(x0_h, x0_w), mode='bilinear', align_corners=True), \
            F.interpolate(y_list[2], size=(x0_h, x0_w), mode='bilinear', align_corners=True), \
            F.interpolate(y_list[3], size=(x0_h, x0_w), mode='bilinear', align_corners=True)], 1)
